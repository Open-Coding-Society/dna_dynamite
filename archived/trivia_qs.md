...
<h3> Practice your trivia skills below by selecting the three hearts! Reload for a new question. </h3>
...
<script>
  ...
  function loadQuiz() {
    ...
    selectedQuestions.forEach((question, index) => {
      let questionHTML = `<div class='question' id='q${index}'><p>${question.q}</p>`;
    ...
    selectedQuestions.forEach((question, index) => {
      ...
      if (selectedOption) {
        if (selectedOption.value === question.answer) {
          ...
          feedback.innerHTML = `<span class='incorrect'>❌ Incorrect. ${question.explanation}</span>`;
          feedback.classList.add("incorrect");
        }
      } else {
        feedback.innerHTML = `<span class='incorrect'>❌ No answer selected. ${question.explanation}</span>`;
        feedback.classList.add("incorrect");
      }
    });
  }
